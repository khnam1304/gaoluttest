// Override Ko:
ko.bindingHandlers.textNum = {
    update: function (element, valueAccessor) {
        var value = valueAccessor();
        if (value && value.toString().indexOf("-") == -1) {
            value = Number(value);
            $(element).text("+ " + value.format());
        }
        else if (value == null)
            $(element).text("0.00");
...
NBList.DataGridAjax = (function () {
    ...
    function dataGridAjax(api, categoryId, pageSizeInit, renderTo, picSize) {
        var preload = setTimeout(function () {
            $("#nb-list-wrapper").show(); clearTimeout(preload);
        });
        var params = picSize || { picWidth: 90, picHeight: 138 };
        ...
        params.sortType = NBList.getLs("sortType", ""); // norder_desc/price_asc/price_desc/product_desc/view_desc
        ...
        self.GridParams = {
            ...
            picWidth: ko.observable(params.picWidth),
            picHeight: ko.observable(params.picHeight),
