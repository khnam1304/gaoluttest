...
PagedFeedBackLoaded = function () {
    ...
    function replaceValidationUi(form) {
        ...
        submitButton.bind("click", function (event) {
            ...
            for (i = 0; i < invalidFields.length; i++) {
                ...
                parent.insertAdjacentHTML("beforeend", "<div class='error-message tooltiptext'>" + Resources._(invalidFields[i].validationMessage) + "</div>");
            }
    ...
    if ($(".submit-feedback-successful").length > 0) {

        $("form.site-form input[name=email]").val("");
        window.showNotify("success", $(".submit-feedback-successful").html());

        $(".submit-feedback-successful").remove();

        $.each($("form.site-form .icon input"), function (k, v) {
            if ($(v).val() != "" && $(v).val() != undefined) {
                $(v).parent().find("label").addClass("float");
            }
        });
        //window.location.href = "/thank.html";

    } else if ($(".submit-feedback-error").length > 0) {

        window.showNotify("success", $(".submit-feedback-error").html());
        $(".submit-feedback-error").remove();
    }
    ...
    $("input,textarea,select").on("change keyup paste", function () {
        if ($("#Custom").length > 0) {
            ...
            $.each($("input[data-scope=Custom],select[data-scope=Custom]"), function (k, v) {
                var key = $(v).attr("name");
                // $('input[name=ContactPreference]:checked').val()
                if (key !== "DataCustom") {
